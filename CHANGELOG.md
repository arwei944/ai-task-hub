...
从 v2 单体架构全面迁移到 v3 内核化架构，引入 DI 容器、能力注册、自愈系统和运维面板。

#### 核心架构

- **AppKernel 内核** — 7 Capability 模块化注册，322ms 冷启动
- **DI Container** — 依赖注入容器，32 个服务全类型化
- **ServiceRegistry** — 完整类型接口，零 `any` 泛型推断
- **V3 EventBus** — 类型安全的事件总线，支持 TTL 和持久化
- **LinkageTracer** — 跨模块联动追踪，拓扑热力图

#### 运维面板 (8 视图)

- **总览仪表盘** — 7 能力健康卡片 + 熔断器 + 死信队列 + 实时统计
...
- **TypeScript 错误**: 737 → 3 (src/ 目录 0 错误)
- **测试**: 101 套件 / 1886 测试通过
- **Prisma Client** 自动生成
- **Vitest** 测试框架
...
从 alpha.1 到 beta.1，历经 3 个迭代版本，AI Task Hub v2.3.0 正式发布。本版本聚焦工作流引擎进化、通知系统个性化和前端体验完善。

#### 工作流引擎 v3 (Phase 1)
- **SubWorkflowStep** — 调用另一个工作流，支持输入/输出映射和上下文继承
- **DynamicStep** — 运行时动态添加步骤，支持从 config/上下文/模板生成
- **ExecutionStateManager** — 执行状态持久化，支持断点保存和恢复
- **Condition 步骤修复** — executor 现在正确执行分支步骤（之前只返回不执行）
- **14 种步骤类型**（新增 sub-workflow + dynamic-step）
- **7 个新 MCP 工具** — 步骤类型列表、执行状态、暂停/恢复、子工作流调用

#### 通知系统增强 (Phase 2)
- **NotificationPreferenceService** — 用户级通知偏好（事件订阅、渠道选择、最低级别）
- **NotificationAggregator** — 通知去重和聚合（5 秒时间窗口内相似通知合并）
- **shouldDeliver()** — 智能投递判断（静默时段、自身操作过滤、事件模式匹配、级别过滤）
- **NotificationPreference Prisma 模型** — 持久化用户偏好设置
- **3 个新 MCP 工具** — 获取/设置偏好、检查投递条件

#### 前端页面完善 (Phase 3)
- **部署管理页面** — 3 个 Tab（概览/部署记录/环境管理）
- **环境健康状态** — 彩色圆点指示器（绿/黄/红/灰）
- **部署状态徽章** — 9 种状态颜色编码
- **策略中文标签** — 滚动更新/蓝绿部署/金丝雀/重建部署
- **侧边栏导航** — 新增"部署管理"入口
...
### v2.2.0 正式发布 — "Platform Pulse"

从 alpha.1 到 beta.2，历经 5 个迭代版本，AI Task Hub v2.2.0 正式发布。本版本聚焦平台可观测性、事件可靠性和外部集成能力。

#### 仪表盘增强 (Phase 1)
- **项目健康度评分** — 综合任务完成率、逾期数、近期活动、阶段进度计算 0-100 分
- **部署统计** — 按状态/环境/策略分组、成功率、平均耗时、环境健康状态
- **Agent 效率指标** — 操作统计、成功率、Top Agent 排行、按客户端类型分组
- **事件总线指标** — 按领域分组、Top 事件类型、每小时吞吐量
- **9 个新 MCP 工具** — 覆盖所有仪表盘数据
- **5 个新 tRPC 端点** — projectHealth, deploymentStats, agentEfficiency, eventMetrics, projectDashboard
- **修复 SystemStats 硬编码** — 模块数和集成数现在从数据库动态获取

#### 通知规则持久化 (Phase 2)
- **NotificationRule Prisma 模型** — 数据库持久化通知规则
- **规则 CRUD** — 创建/列表/更新/删除，支持优先级排序
- **动态规则加载** — 模块启用时自动从数据库加载规则
- **渠道定向** — 规则可指定目标通知渠道
- **10 个新事件监听** — deployment(5), lifecycle(3), knowledge(2)
- **6 个新 MCP 工具** — 通知规则管理

#### EventBus 可靠性增强 (Phase 3)
- **死信队列 (DLQ)** — 失败事件自动捕获，最大 1000 条
- **指数退避重试** — 1s → 4s → 16s → 64s，最多 3 次重试
- **事件指标** — 发射/处理/失败/重试计数，按类型分组
- **事件 TTL** — 自动清理过期事件（默认 7 天）
- **DLQ 管理** — 查看/重试/移除/清空操作
- **6 个新 MCP 工具** — EventBus 监控和 DLQ 管理

#### 出站 Webhook (Phase 4)
- **独立出站服务** — 事件驱动的 Webhook 转发
- **Webhook CRUD** — 创建/列表/更新/删除
- **事件模式筛选** — 支持通配符匹配
- **HMAC-SHA256 签名** — 安全投递
- **投递日志** — 请求/响应/状态码/耗时/错误
- **2 个新 Prisma 模型** — OutboundWebhook, OutboundWebhookDelivery
- **6 个新 MCP 工具** — 出站 Webhook 管理

#### tRPC API 完善 (Phase 5)
- **2 个新路由** — deployments (4 端点), notificationRules (4 端点)
- **14 个 tRPC 路由** — 覆盖所有核心模块
