...
export class ProjectHubService {
  ...
  /**
   * 创建项目并注册智能体身份
   * 原子化操作：创建项目 → 创建 Agent → 分配 Agent → 自动创建工作日志
   * @param data - 创建数据（含项目信息 + Agent 身份）
   * @returns 创建结果（project, agent, projectAgent）
   */
  async createProjectWithAgent(data: {
    // Project info
    name: string;
    description?: string;
    priority?: string;
    ...
    this.logger.info('[ProjectHubService] Creating project with agent', {
    ...
    const agent = await this.prisma.agent.create({
      data: {
        name: data.agentName,
        description: `${data.agentName} — ${data.agentRole || '全栈开发'}`,
        apiKey: `auto_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`,
        clientType: data.agentClientType || 'claude',
        capabilities: data.agentCapabilities ? JSON.stringify(data.agentCapabilities) : null,
        isActive: true,
      },
    });
    ...
    // 4. Auto-create initial work log
    await this.prisma.agentWorkLog.create({
      data: {
        projectId: project.id,
        projectAgentId: projectAgent.id,
        date: new Date().toISOString().split('T')[0],
        hours: 0,
        description: `🤖 ${data.agentName} 注册身份并加入项目`,
        autoGenerated: true,
      },
    });
  ...
  async updateProject(id: string, data: UpdateProjectDTO) {
    ...
    // Auto-create work log when phase changes
    if (data.phase && existing.phase !== data.phase) {
      try {
        const leadAgent = await this.prisma.projectAgent.findFirst({
          where: { projectId: id, role: 'lead', isActive: true },
        });
        if (leadAgent) {
          await this.prisma.agentWorkLog.create({
            data: {
              projectId: id,
              projectAgentId: leadAgent.id,
              date: new Date().toISOString().split('T')[0],
              hours: 0,
              description: `项目阶段变更: ${existing.phase} → ${data.phase}`,
              autoGenerated: true,
            },
          });
        }
      } catch (err) {
        this.logger.warn('[ProjectHubService] Failed to auto-create work log for phase change', { err });
      }
    }
