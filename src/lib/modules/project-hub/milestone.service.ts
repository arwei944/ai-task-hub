...
export class MilestoneService {
  ...
  async update(id: string, data: UpdateMilestoneDTO) {
    ...
    // Auto-create work log when milestone is completed
    if (data.status === 'completed' && existing.status !== 'completed') {
      try {
        // Find the lead agent for this project
        const leadAgent = await this.prisma.projectAgent.findFirst({
          where: { projectId: existing.projectId, role: 'lead', isActive: true },
        });
        if (leadAgent) {
          await this.prisma.agentWorkLog.create({
            data: {
              ...
              hours: 0,
              description: `完成里程碑: ${existing.title}`,
              autoGenerated: true,
            },
          });
        }
      } catch (err) {
        // Non-fatal: work log creation should not block milestone update
        this.logger.warn('[MilestoneService] Failed to auto-create work log', { err });
      }
    }
