  ...
  async execute(config: Record<string, unknown>, context: Record<string, unknown>) {
    ...
    const promises = subSteps.map((step, index) => {
      ...
      return this.deps.executor.executeStep({
        ...
        contextManager: {
          getAll: () => subContext,
          merge: (result: StepResult) => { Object.assign(subContext, result); },
    ...
    const parallelResults = results.map((r, i) => ({
      index: i,
      stepName: subSteps[i]?.name ?? `parallel-${i}`,
    ...
    if (!allSuccess) {
      const failedCount = results.filter(r => r.status === 'rejected' || (r.status === 'fulfilled' && r.value.status !== 'completed')).length;
      throw new Error(`Parallel group: ${failedCount}/${subSteps.length} sub-steps failed`);
    }
