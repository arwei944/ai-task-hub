...
export class InvokeAgentStep implements StepHandler {
  ...
  async execute(config: Record<string, unknown>, context: Record<string, unknown>) {
    if (!this.deps.soloBridge) {
      throw new Error('SOLO Bridge is required for invoke-agent step');
    }
    ...
    const timeoutMs = Number(config.timeoutMs) || 120000; // 默认 2 分钟
