...
export class AIAnalyzeStep implements StepHandler {
  ...
  async execute(config: Record<string, unknown>, context: Record<string, unknown>) {
    ...
    const result = await this.deps.soloBridge.call({
      ...
      stepName: String(context._stepName ?? 'ai-analyze'), subAgentType: subAgentMap[mode] ?? 'explore',
      sessionId: context._soloSessionId as string | undefined, context, timeoutMs: Number(config.timeoutMs) || undefined,
    });
