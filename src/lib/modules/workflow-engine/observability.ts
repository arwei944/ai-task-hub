...
export class Observability {
  ...
  recordStepMetrics(params: {
    ...
    this.stepMetrics.push(record);
    if (this.stepMetrics.length > this.maxEntries) {
      this.stepMetrics.shift();
    }
  }

  /** 记录 SOLO 调用（增强：同时存入内存） */
  recordSOLOCall(record: SOLOCallRecord): void {
    ...
    this.soloCallHistory.push(record);
    if (this.soloCallHistory.length > this.maxEntries) {
      this.soloCallHistory.shift();
    }
  }

  /** 记录执行开始 */
  recordExecutionStart(params: {
    executionId: string;
    workflowId: string;
    workflowName: string;
    triggerType: string;
    ...
    const record: ExecutionMetricRecord = {
      id: this.generateId(),
      executionId: params.executionId,
    ...
    if (this.executionHistory.length > this.maxEntries) {
      this.executionHistory.shift();
    }

    this.emitWorkflowEvent('execution.started', {
      executionId: params.executionId,
      workflowId: params.workflowId,
      workflowName: params.workflowName,
      triggerType: params.triggerType,
  ...
  recordExecutionEnd(params: {
    ...
    const record = this.executionHistory.find(
      (r) => r.executionId === params.executionId && r.status === 'running',
    );

    if (record) {
      record.status = params.status;
      record.completedSteps = params.completedSteps;
      record.failedSteps = params.failedSteps;
      record.totalTokensUsed = params.totalTokensUsed;
      record.durationMs = Date.now() - record.startedAt.getTime();
      record.completedAt = new Date();
    }

    this.emitWorkflowEvent('execution.completed', {
      executionId: params.executionId,
      status: params.status,
  ...
  getStepMetrics(filter?: {
    ...
    if (filter?.stepType) {
      results = results.filter((r) => r.stepType === filter.stepType);
    }
    if (filter?.status) {
  ...
  getSOLOCallHistory(filter?: {
    ...
    if (filter?.stepId) {
      results = results.filter((r) => r.stepId === filter.stepId);
    }
    if (filter?.limit && filter.limit > 0) {
      results = results.slice(-filter.limit);
    }
  ...
  getWorkflowStats(workflowId: string): {
    ...
    const avgDurationMs =
      completedExecutions.length > 0
        ? completedExecutions.reduce((sum, r) => sum + r.durationMs, 0) / completedExecutions.length
        : 0;

    const withTokens = completedExecutions.filter((r) => r.totalTokensUsed != null && r.totalTokensUsed > 0);
    const avgTokensUsed =
      withTokens.length > 0
        ? withTokens.reduce((sum, r) => sum + (r.totalTokensUsed ?? 0), 0) / withTokens.length
        : 0;
  ...
  getGlobalStats(): {
    ...
    const totalSteps = this.stepMetrics.length;
    const totalSOLOCalls = this.soloCallHistory.length;

    const avgStepDurationMs =
      totalSteps > 0
        ? this.stepMetrics.reduce((sum, r) => sum + r.durationMs, 0) / totalSteps
        : 0;

    const finishedExecutions = this.executionHistory.filter(
      (r) => r.status === 'completed' || r.status === 'failed',
    );
    const successRate =
      finishedExecutions.length > 0
        ? finishedExecutions.filter((r) => r.status === 'completed').length / finishedExecutions.length
        : 0;
