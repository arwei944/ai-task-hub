...
export class WorkflowOrchestrator {
  ...
  constructor(private prisma: PrismaClient, private executor: WorkflowExecutor, private concurrencyController: ConcurrencyController, private observability: Observability, private logger?: Logger) {}

  async startExecution(params: { workflowId: string; workflowName: string; steps: WorkflowStep[]; variables?: Record<string, unknown>; triggerType?: string; triggeredBy?: string; retryPolicy?: RetryPolicy; timeoutMs?: number; }): Promise<{ executionId: string; status: string }> {
    ...
    const execution = await this.prisma.workflowExecution.create({ data: { workflowId, workflowSnapshot: JSON.stringify({ id: workflowId, name: workflowName, steps, variables }), status: 'running', triggerType: triggerType ?? 'manual', triggeredBy: triggeredBy ?? null, startedAt: new Date(), context: variables ? JSON.stringify(variables) : '{}' } });
    this.runningExecutions.set(execution.id, true);
    this.runExecution(execution.id, steps, variables, timeoutMs).catch(err => { this.logger?.error(`Workflow execution failed: ${execution.id}`, { error: String(err) }); });
  ...
  setTriggerDispatcher(dispatcher: any): void { this.triggerDispatcher = dispatcher; }
  ...
  private async runExecution(executionId: string, steps: WorkflowStep[], variables?: Record<string, unknown>, timeoutMs?: number): Promise<void> {
    ...
    try {
      ...
      if (timeoutMs) { timeoutHandle = setTimeout(async () => { this.runningExecutions.set(executionId, false); await this.prisma.workflowExecution.update({ where: { id: executionId }, data: { status: 'failed', error: `Workflow timed out after ${timeoutMs}ms`, completedAt: new Date() } }); }, timeoutMs); }
    ...
    } finally { this.runningExecutions.delete(executionId); this.concurrencyController.release(); }
  }
}