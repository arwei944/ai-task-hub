...
export const projectHubMcpTools: McpToolConfig[] = [
  ...
  },
  {
    ...
    description: '列出所有项目。智能体用于查看可用的项目列表，支持按状态、优先级筛选和搜索。',
    inputSchema: {
      type: 'object',
      properties: {
        status: { type: 'string', enum: ['active', 'paused', 'completed', 'archived'], description: '按状态筛选' },
  ...
  {
    name: 'ph_manage_docs',
    description: '管理项目文档。智能体用于创建、更新、搜索项目文档，支持需求文档、设计文档、技术文档等类型。',
    inputSchema: {
      type: 'object',
      properties: {
        projectId: { type: 'string', description: '项目 ID' },
        action: { type: 'string', enum: ['create', 'update', 'list', 'search'], description: '操作类型' },
        docId: { type: 'string', description: '文档 ID（update 时必填）' },
        title: { type: 'string', description: '文档标题（create 时必填）' },
        content: { type: 'string', description: '文档内容（Markdown 格式）' },
        docType: { type: 'string', enum: ['requirement', 'design', 'technical', 'meeting_notes', 'general'], description: '文档类型' },
        queryText: { type: 'string', description: '搜索关键词（search 时必填）' },
      },
      required: ['projectId', 'action'],
    },
  },
