    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex gap-2 mb-6">
          <button onClick={() => setActiveTab('pending')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'pending' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700'}`}>
            待处理 {pendingCount > 0 && `(${pendingCount})`}
          </button>
          <button onClick={() => setActiveTab('all')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'all' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700'}`}>
            全部
          </button>
        </div>
        {loading ? (
          ...
          <div className="space-y-3">
            {filteredCheckpoints.map((cp) => {
              ...
              return (
                <div key={cp.id} onClick={() => setSelectedCheckpoint(isSelected ? null : cp)} className={`bg-white dark:bg-gray-900 rounded-xl shadow-sm border cursor-pointer transition-all ${isSelected ? 'border-blue-500 ring-2 ring-blue-100 dark:ring-blue-900' : 'border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700'}`}>
                  <div className="p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{cp.checkpointType === 'pre_execute' ? '执行前检查' : '执行后检查'} · {new Date(cp.createdAt).toLocaleString()}</p>
                      {cp.intervention && <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{cp.intervention}</p>}
                    </div>
                    {cp.status === 'pending' && (
                      <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => handleApprove(cp.id)} className="px-3 py-1.5 text-xs font-medium bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">批准</button>
                        <button onClick={() => handleReject(cp.id)} className="px-3 py-1.5 text-xs font-medium bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">拒绝</button>
                      </div>
                    )}
